...
describe PropLogic::Sat4j::IncrementalSolver do
  ...
  expected_methods.each do |sym|
    it "has ##{sym} method" do
      expect(PropLogic::Sat4j::IncrementalSolver.public_instance_methods).to include sym
    end
  end

  describe '#add' do
    it 'can add one term properly' do
      s = PropLogic::Sat4j::IncrementalSolver.new a
      s.add(b)
